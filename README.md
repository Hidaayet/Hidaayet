# 👋 Hi, I'm Hidayet Yaakoubi

## 🩺 About Me
- BME Engineer specializing in deep learning for medical applications
- Building AI models that solve real clinical problems — from diabetic retinopathy detection to CRISPR off-target prediction
- Based in Tunisia 🇹🇳

## 🔬 Featured Projects

### 🧠 Motor Imagery BCI
EEG-based brain-computer interface for motor imagery classification. Achieved 72.4% accuracy on BCI Competition IV dataset.
- [Repository]([link](https://github.com/Hidaayet/motor-imagery-bci)) | [Paper PDF](link)

### 👁️ Retinal Disease Detector
EfficientNet-B3 for diabetic retinopathy detection. QWK 0.9053, deployed on HuggingFace.
- [Live Demo](link) | [Repository](link)

### 🧬 CRISPR Off-Target Predictor
Transformer-based model for predicting off-target cleavage sites. AUC 0.9711.
- [Repository](link)

### 🏥 Mouna - Breast Cancer Risk Assessment
Non-invasive risk assessment using questionnaire + blood biomarkers. AUC 0.926 on 244K patients.
- [Repository](link)

## 🛠️ Tech Stack
- **Deep Learning:** PyTorch, TensorFlow, EEGNet, EfficientNet, Transformers, GNN
- **Signal Processing:** MNE, NumPy, SciPy
- **Medical Imaging:** OpenCV, PIL, DICOM
- **Tools:** Jupyter, Git, Docker, HuggingFace

## 📫 Connect
- LinkedIn: [link]
- Email: enghideya@gmail.com (or better: hideya@domain.com)
- Twitter: [link]

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&theme=radical)

## 📫 Reach me at
[enghideya@gmail.com]

